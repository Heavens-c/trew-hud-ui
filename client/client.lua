...
Citizen.CreateThread(function()
	while true do
		...
		-- 	-- Identifier
		-- 	local idMessageString = string.format(
		-- 		Locales[Config.Locale]['your_id'],
		-- 		GetPlayerServerId(PlayerId())
		-- 	)
...
local vehicleLockStatus = 'unlocked'
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsPedInAnyVehicle(player, false) and vehicleIsOn then
			...
			-- Vehicle Fuel, Engine and Gear
			local vehicleFuel
			vehicleFuel = GetVehicleFuelLevel(vehicle)
			local vehicleEngine
			vehicleEngine = (GetVehicleEngineHealth(vehicle) * 100) / 1000
			...
			vehicleInfo = {
				...
				locks = GetVehicleDoorLockStatus(vehicle),
...
AddEventHandler('playerSpawned', function()
		...
		if ALCOOL == false then
			exports.trew_hud_ui:createStatus({
			    status = 'drunk',
			    color = '#00fff2',
			    icon = '<i class="fas fa-cocktail"></i>'  --icons https://fontawesome.com/icons?d=gallery
			});
			ALCOOL = true
		end
...
RegisterCommand('toggleui', function()
	if not toggleui then
		...
		SendNUIMessage({ action = 'element', task = 'disable', value = 'identifier' })
	else
		...
		if (Config.ui.showId == true) then
			SendNUIMessage({ action = 'element', task = 'enable', value = 'identifier' })
		end
	end
