...
window.onload = function() {
    var eventCallback = {
        ui: function(data) {
            ...
            if (config.showServerLogo == true) {
                document.querySelector(".info.server").style.display = "block";
            } else if (config.showServerLogo == false) {
                document.querySelector(".info.player").style.top = "55px";
            }
            ...
            if (config.showSocietyMoney == true) {
                document.querySelector("#society").style.display = "block";
            }
            ...
            } else if (config.showVoice == false) {
                document.querySelector(".info.time-and-place").style.right =
                    "25px";
            }
        ...
        element: function(data) {
            if (data.task == "enable") {
                document.querySelector("#" + data.value).style.display =
                    "block";
        ...
        setLogo: function(data) {
            document
                .querySelector("#server img")
                .setAttribute("src", data.value);
        },

        setMoney: function(data) {
            data.value = data.value.toLocaleString();
            var oldValue = document.querySelector("#" + data.id + " span")
                .innerHTML;
            ...
            if (data.isdead == true) {
                ...
                for (i = 0; i < data.status.length; i++) {
                    if (
                        ...
                        data.status[i].name == "drug"
                    ) {
                        var statusValue = Math.floor(
                            100 - data.status[i].value
                        );
                    } else {
                ...
                if (
                    document.querySelector("#health").classList.contains("dead")
                ) {
        ...
        updateVehicle: function(data) {
            ...
            var vehicleLocks = document.querySelector("#locks");
            var vehicleFuel = document.querySelector("#fuel");
            var vehicleEngine = document.querySelector("#engine");
            var vehicleCruiser = document.querySelector(
                "#vehicle-speed strong"
            );
            var vehiclesCars = [
                0,
                1,
            ...
            if (data.status == true) {
                if (vehicleInfo.classList.contains("inactive")) {
                    ...
                    vehicleLocks.style.display = "none";
                ...
                if (vehicleInfo.classList.contains("updated") == false) {
                    ...
                    var vehicleAverageSpeed = Math.ceil(
                        data.config.maxSpeed / 6
                    );
                ...
                if (previousGear != currentGear) {
                    document
                        .querySelector("#vehicle-gear")
                        .classList.add("pulse");
                }
                saferInnerHTML(
                    document.querySelector("#vehicle-gear span"),
                    data.gear
                );
                ...
                if (
                    ...
                    eventCallback.sound("sounds/seatbelt-buckle.ogg", {
                        volume: "0.50"
                    });
                } else if (
                ...
                if (vehicleLights.classList.contains(data.lights) == false) {
                    ...
                    if (data.lights == "high") {
                        ...
                        vehicleLights.querySelector("i img").src =
                            "img/vehicle-lights.png";
                    }
                }

                if (vehicleSignals.classList.contains(data.signals) == false) {
                    vehicleSignals.querySelector("i").className =
                        "fa fa-arrows-alt-h";
                    vehicleSignals.classList.remove(
                        "left",
                        "right",
                        "both",
                        "off"
                    ...
                    if (
                        ...
                        vehicleSignals
                            .querySelector("i")
                            .classList.add("fas", classSignal);
                        vehicleSignals.classList.add(data.signals, "dying");

                        eventCallback.sound("sounds/car-indicators.ogg", {
                            loop: "loop"
                        });
                ...
                if (
                    data.locks == 1 && // unlocked
                    vehicleLocks.classList.contains("locked")
                ) {
                    vehicleLocks.classList.remove("locked");
					vehicleLocks.classList.add("unlocked");
					vehicleLocks.querySelector("i").className =
                        "fas fa-lock-open";
                } else if (
                    data.locks == 2 && // locked
                    vehicleLocks.classList.contains("unlocked")
                ) {
                    vehicleLocks.classList.remove("unlocked");
					vehicleLocks.classList.add("locked");
					vehicleLocks.querySelector("i").className =
                        "fas fa-lock";
                }

                vehicleFuel.querySelector("span").style.height =
                    data.fuel + "%";
				vehicleEngine.querySelector("span").style.height =
					data.engine + "%";
				...
				if (data.engine <= 40) {
					if (vehicleEngine.classList.contains("dying") == false) {
						vehicleEngine.classList.add("dying");
					}
				} else {
					vehicleEngine.classList.remove("dying");
				}
            } else {
